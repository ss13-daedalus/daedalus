proc/sql_poll_players()
	...
	if(!dbcon.IsConnected())
		...
		var/SQLite/Query/query = dbcon.NewQuery("INSERT INTO population (playercount, time) VALUES ([playercount], '[sqltime]')")
		if(!query.Execute())
...
proc/sql_poll_admins()
	...
	if(!dbcon.IsConnected())
		...
		var/sqltime = time2text(world.realtime, "YYYY-MM-DD hh:mm:ss")
		var/SQLite/Query/query = dbcon.NewQuery("INSERT INTO population (admincount, time) VALUES ([admincount], '[sqltime]')")
		if(!query.Execute())
			var/err = query.ErrorMsg()
...
proc/sql_report_death(var/mob/living/carbon/human/H)
	...
	if(!dbcon.IsConnected())
		...
		var/SQLite/Query/query = dbcon.NewQuery("INSERT INTO death (name, byondkey, job, special, pod, tod, laname, lakey, gender, bruteloss, fireloss, brainloss, oxyloss, coord) VALUES ('[sqlname]', '[sqlkey]', '[sqljob]', '[sqlspecial]', '[sqlpod]', '[sqltime]', '[laname]', '[lakey]', '[H.gender]', [H.getBruteLoss()], [H.getFireLoss()], [H.brainloss], [H.getOxyLoss()], '[coord]')")
		if(!query.Execute())
			var/err = query.ErrorMsg()
...
proc/sql_commit_feedback()
	...
	if(!dbcon.IsConnected())
		...
		var/SQLite/Query/max_query = dbcon.NewQuery("SELECT MAX(roundid) AS max_round_id FROM erro_feedback")
		max_query.Execute()

		var/newroundid

		while(max_query.NextRow())
			newroundid = max_query.item[0]
		...
		for(var/datum/feedback_variable/item in content)
			...
			var/SQLite/Query/query = dbcon.NewQuery("INSERT INTO erro_feedback (roundid, time, variable, value) VALUES ([newroundid], datetime('now'), '[variable]', '[value]')")
			if(!query.Execute())
				var/err = query.ErrorMsg()
				log_game("SQL ERROR during death reporting. Error : \[[err]\]\n")
...
proc/debug_sql_commit_feedback()
	...
	for(var/datum/feedback_variable/item in content)
		...
		world << "INSERT INTO erro_feedback (roundid, time, variable, value) VALUES (ABC, datetime('now'), '[variable]', '[value]')"
